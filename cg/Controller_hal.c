/*
 * Hardware configuration file for: TI28P65x
 * Generated with                 : PLECS 4.9.5
 * Generated on                   : Sun Feb  8 22:44:51 2026
 */
...
// Clock configuration
#define PLX_DEVICE_SETCLOCK_CFG       (SYSCTL_OSCSRC_OSC2 | SYSCTL_IMULT(40) | \
                                       SYSCTL_REFDIV(1) | SYSCTL_ODIV(2) | \
                                       SYSCTL_SYSDIV(1) | SYSCTL_PLL_ENABLE | \
                                       SYSCTL_DCC_BASE_0)

#define SYSCLK_HZ 200000000L
#define LSPCLK_HZ (200000000L / 4l)

#define PLX_DELAY_US(A)  F28x_usDelay(((((long double) A * \
                                         1000.0L) / \
                                        (long double)5.000000L) - \
                                       9.0L) / 5.0L)
...
PIL_CONST_DEF(unsigned char, CompiledDate[], "02/08/2026 10:44 PM");
...
{
   uint16_t counts = (uint16_t)(fminf(65535.0f, delay * 2.000000e+08f));
...
void Controller_initHal()
{
   ...
   PIL_setAndConfigScopeBuffer(PilHandle, (uint16_t *)&ScopeBuffer, 1020, 10);
   ...
   PLX_SCI_configure(SciHandles[0],
                     PLX_SCI_SCI_A, 50000000);

   (void)PLX_SCI_setupPort(SciHandles[0], 115200);
   {
      // early system configuration
      PLX_DIO_sinit();
      ASysCtl_selectCMPHPMux(ASYSCTL_CMPHPMUX_SELECT_10, 1);
   }
   {
      SysCtl_enablePeripheral(SYSCTL_PERIPH_CLK_CMPSS10);
      CMPSS_enableModule(CMPSS10_BASE);
      CMPSS_configHighComparator(CMPSS10_BASE, CMPSS_INSRC_DAC);
      CMPSS_configDACHigh(CMPSS10_BASE,  CMPSS_DACVAL_SYSCLK |
                          CMPSS_DACSRC_SHDW);
      CMPSS_setDACValueHigh(CMPSS10_BASE, 3270);             // 2.634146 V
      CMPSS_configFilterHigh(CMPSS10_BASE, 0, 1, 1);
      CMPSS_initFilterHigh(CMPSS10_BASE);

      CMPSS_configOutputsHigh(CMPSS10_BASE, CMPSS_TRIP_FILTER);

      // low comparator not used
   }

   {
      PLX_AIN_sinit(200000000);
   ...
   // configure SOC1 of ADC-B to measure ADCIN6
   {
      ...
      // set SOC trigger to PWM2
      params.ADCSOCxCTL.bit.TRIGSEL = 7;
      params.ADCSOCxCTL.bit.ACQPS = 14;

      PLX_AIN_setupChannel(AdcHandles[1], 0, 6, &params);
   }
   ...
   // configure SOC1 of ADC-A to measure ADCIN10
   {
      PLX_AIN_ChannelParams_t params;
      PLX_AIN_setDefaultChannelParams(&params);
      ...
      PLX_AIN_setupChannel(AdcHandles[0], 0, 10, &params);
   }

   // configure SOC2 of ADC-A to measure ADCIN8
   {
      PLX_AIN_ChannelParams_t params;
      PLX_AIN_setDefaultChannelParams(&params);
      ...
      PLX_AIN_setupChannel(AdcHandles[0], 1, 8, &params);
   }

   {
      ...
      PLX_DIO_configureOut(doutHandle, 13, &props);
   ...
   // soc='p'
   {
      ...
      PLX_PWM_configure(EpwmHandles[0], 2, &params);
      // configure deadtime to 1.000000e-08/1.000000e-08 seconds (rising/falling)
      PLX_PWM_setDeadTimeCounts(EpwmHandles[0], 2, 2);
      ...
      EPwm2Regs.ETSOCPS.bit.SOCAPRD2 = 3;
      EPwm2Regs.ETSEL.bit.SOCAEN = 1;

      EPWM_setTripZoneDigitalCompareEventCondition(EPWM2_BASE,
                                                   EPWM_TZ_DC_OUTPUT_A1,
                                                   EPWM_TZ_EVENT_DCXH_HIGH);
      EPWM_enableDigitalCompareTripCombinationInput(EPWM2_BASE,
                                                    EPWM_DC_COMBINATIONAL_TRIPIN4,
                                                    EPWM_DC_TYPE_DCAH);

      EPWM_setDigitalCompareEventSource(EPWM2_BASE,
                                        EPWM_DC_MODULE_A,
                                        EPWM_DC_EVENT_1,
...

   {
      ...
      PLX_DIO_configureOut(DoutHandles[0], 99,  &props);
   }

   {
      ...
      props.enableInvert = false;
      PLX_DIO_configureOut(DoutHandles[1], 12,  &props);
   }

   DISPR_sinit();
   DISPR_configure((uint32_t)(3000), PilHandle, &TaskObj[0],
   ...
   {
      static int taskId = 0;
      // Task 0 at 6.666667e+04 Hz
      DISPR_registerTask(0, &Tasks, 3000L, (void *)&taskId);
   }
   {
      static int taskId = 1;
      // Task 1 at 2.000020e+00 Hz
      DISPR_registerTask(1, &Tasks, 99999000L, (void *)&taskId);
   }
   EALLOW;
   AdcaRegs.ADCINTSEL1N2.bit.INT1CONT = 0;  // disable ADCINT1 Continuous mode
   AdcaRegs.ADCINTSEL1N2.bit.INT1SEL = 1;  // setup EOC1 or OSINT(1 - 15) to trigger ADCINT1
   ...
   *(PINT *)((uint32_t)(&PieVectTable.ADCA1_INT) + ((uint32_t)0)*
...

   {
      ...
      XBAR_setEPWMMuxConfig(XBAR_TRIP4, XBAR_EPWM_MUX60_CMPSS10_CTRIPH);
      XBAR_enableEPWMMux(XBAR_TRIP4, XBAR_MUX60);
